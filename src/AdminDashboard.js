...
const tableWrap = { background:'#141414', border:'1px solid #222', borderRadius:10, overflowX:'auto', WebkitOverflowScrolling:'touch', marginBottom:14 }
const tableStyle = { width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:550 }
const thStyle = { textAlign:'left', padding:'9px 12px', fontSize:11, fontWeight:600, color:mu, borderBottom:'1px solid #222', letterSpacing:1, textTransform:'uppercase', whiteSpace:'nowrap' }
const tdStyle = { padding:'9px 12px', color:w, whiteSpace:'nowrap' }
...
export default function AdminDashboard({ currentUser, products, sales, staffList, page, setPage, onDeleteSale, onUpdateSale, onAddProduct, onUpdateProduct, onDeleteProduct, onAddStaff, onRemoveStaff, onLogout }) {
  ...
  function MetricCard({ label, value, sub, color }) {
    return (
      <div style={{ background:k2, border:'1px solid #222', borderRadius:10, padding:14, minWidth:130 }}>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0A0A0A', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:16, flex:1, overflowX:'hidden' }}>

        {editSale && <EditSaleModal sale={editSale} onSave={updates => { onUpdateSale(editSale.id, updates); setEditSale(null); setPage('sales') }} onCancel={() => setEditSale(null)} />}
        {confirmDel && <ConfirmDeleteModal sale={confirmDel} onConfirm={() => { onDeleteSale(confirmDel.id); setConfirmDel(null) }} onCancel={() => setConfirmDel(null)} />}
        {editProd && <EditProductModal product={editProd} onSave={updates => { onUpdateProduct(editProd.id, updates); setEditProd(null); setPage('stock') }} onCancel={() => setEditProd(null)} />}
            ...
            <div style={tableWrap}>
              <table style={tableStyle}>
                <thead><tr style={{ background:k3 }}>
                  {['Items','Staff','Total','Profit','Date','Actions'].map(h => <th key={h} style={thStyle}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {sales.slice(0,6).map(s => {
                    ...
                    return (
                      <tr key={s.id} style={{ borderBottom:'1px solid #1A1A1A' }}>
                        <td style={{ ...tdStyle, maxWidth:180, overflow:'hidden', textOverflow:'ellipsis' }} title={items}>{items}</td>
                        <td style={tdStyle}>{s.staff}</td>
                        <td style={tdStyle}>{fmt(s.total)}</td>
                        <td style={{ ...tdStyle, color:gold, fontWeight:600 }}>{fmt(s.profit)}</td>
        ...
        {!editSale && !confirmDel && !editProd && page === 'sales' && <SalesPage sales={sales} onEdit={setEditSale} onDelete={setConfirmDel} />}
        {!editSale && !confirmDel && !editProd && page === 'profit' && <ProfitPage sales={sales} />}
        {!editSale && !confirmDel && !editProd && page === 'staff' && <StaffPage staffList={staffList} onAdd={onAddStaff} onRemove={id => { if(window.confirm('Remove this staff?')) onRemoveStaff(id) }} />}
      </div>
    </div>
  )
}
    ...
    <div style={{ background:'rgba(0,0,0,0.88)', display:'flex', alignItems:'center', justifyContent:'center', padding:20, minHeight:300 }}>
      <div style={{ background:'#141414', border:'1px solid #C9A84C', borderRadius:12, padding:22, width:'100%', maxWidth:400 }}>
        ...
        <div style={{ background:'#1C1C1C', borderRadius:8, padding:12, marginBottom:14, fontSize:13, color:'#EDE8DC' }}>
          ...
          <span style={{ color:'#8A8070' }}>Staff: {sale.staff} | Total: {'₦'+Number(Math.round(sale.total)).toLocaleString()} | {sale.date}</span>
        </div>
...
function AddProductForm({ onAdd, products, onEdit, onDelete, onRestock }) {
  ...
  return (
    <div>
      ...
      <div style={{ background:'#141414', border:'1px solid #222', borderRadius:10, overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:600 }}>
          <thead><tr style={{ background:'#1C1C1C' }}>
            {['Product','Category','Cost','Sell','Qty','Status','Actions'].map(h => <th key={h} style={{ textAlign:'left', padding:'9px 12px', fontSize:11, fontWeight:600, color:'#8A8070', borderBottom:'1px solid #222', letterSpacing:1, textTransform:'uppercase', whiteSpace:'nowrap' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {products.map(p => (
              <tr key={p.id} style={{ borderBottom:'1px solid #1A1A1A' }}>
                <td style={{ padding:'9px 12px', color:'#F5F0E8', fontWeight:600, whiteSpace:'nowrap' }}>{p.name}</td>
                <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{p.category}</td>
                <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(p.cost)).toLocaleString()}</td>
                <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(p.sell)).toLocaleString()}</td>
                <td style={{ padding:'9px 12px', color:'#F5F0E8', fontWeight:700, whiteSpace:'nowrap' }}>{p.qty}</td>
...
function SalesPage({ sales, onEdit, onDelete }) {
  ...
  return (
    <div>
      ...
      <div style={{ background:'#141414', border:'1px solid #222', borderRadius:10, overflowX:'auto', WebkitOverflowScrolling:'touch', marginBottom:14 }}>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:400 }}>
          <thead><tr style={{ background:'#1C1C1C' }}>
            {['Staff','Transactions','Revenue','Profit'].map(h => <th key={h} style={{ textAlign:'left', padding:'9px 12px', fontSize:11, color:'#8A8070', borderBottom:'1px solid #222', letterSpacing:1, textTransform:'uppercase', whiteSpace:'nowrap' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {Object.entries(staffSum).map(([name, s]) => (
              <tr key={name} style={{ borderBottom:'1px solid #1A1A1A' }}>
                <td style={{ padding:'9px 12px', color:'#F5F0E8', fontWeight:600, whiteSpace:'nowrap' }}>{name}</td>
                <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{s.count}</td>
                <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(s.total)).toLocaleString()}</td>
                <td style={{ padding:'9px 12px', color:'#C9A84C', fontWeight:600, whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(s.profit)).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ background:'#141414', border:'1px solid #222', borderRadius:10, overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:600 }}>
          <thead><tr style={{ background:'#1C1C1C' }}>
            {['Items','Staff','Total','Profit','Date','Actions'].map(h => <th key={h} style={{ textAlign:'left', padding:'9px 12px', fontSize:11, color:'#8A8070', borderBottom:'1px solid #222', letterSpacing:1, textTransform:'uppercase', whiteSpace:'nowrap' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {sales.map(s => {
              const items = s.items.map(i => i.name + (i.qty > 1 ? ' x' + i.qty : '')).join(', ')
              return (
                <tr key={s.id} style={{ borderBottom:'1px solid #1A1A1A' }}>
                  <td style={{ padding:'9px 12px', color:'#F5F0E8', whiteSpace:'nowrap', maxWidth:200, overflow:'hidden', textOverflow:'ellipsis' }} title={items}>{items}</td>
                  <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{s.staff}</td>
                  <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(s.total)).toLocaleString()}</td>
                  <td style={{ padding:'9px 12px', color:'#C9A84C', fontWeight:600, whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(s.profit)).toLocaleString()}</td>
                  <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{s.date}</td>
                  <td style={{ padding:'9px 12px', whiteSpace:'nowrap' }}>
...
function ProfitPage({ sales }) {
  ...
  return (
    <div>
      <div style={{ display:'flex', overflowX:'auto', WebkitOverflowScrolling:'touch', gap:10, marginBottom:16, paddingBottom:4 }}>
        {[['Total Revenue', '₦'+Number(Math.round(totalRev)).toLocaleString(), 'All time', '#F5F0E8'],
          ['Total Cost', '₦'+Number(Math.round(totalRev-totalProfit)).toLocaleString(), 'Cost of goods', '#F5F0E8'],
          ['Net Profit', '₦'+Number(Math.round(totalProfit)).toLocaleString(), 'All time', '#6DBF6D'],
          ['Profit Margin', margin+'%', 'Overall', '#C9A84C']].map(([label, val, sub, color]) => (
          <div key={label} style={{ background:'#141414', border:'1px solid #222', borderRadius:10, padding:14, minWidth:140, flexShrink:0 }}>
      ...
      <div style={{ background:'#141414', border:'1px solid #222', borderRadius:10, overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:500 }}>
          <thead><tr style={{ background:'#1C1C1C' }}>
            {['Product','Units Sold','Revenue','Est. Profit','Margin'].map(h => <th key={h} style={{ textAlign:'left', padding:'9px 12px', fontSize:11, color:'#8A8070', borderBottom:'1px solid #222', letterSpacing:1, textTransform:'uppercase', whiteSpace:'nowrap' }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {Object.entries(pg).map(([name, r]) => {
              ...
              return (
                <tr key={name} style={{ borderBottom:'1px solid #1A1A1A' }}>
                  <td style={{ padding:'9px 12px', color:'#F5F0E8', fontWeight:600, whiteSpace:'nowrap' }}>{name}</td>
                  <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{r.qty}</td>
                  <td style={{ padding:'9px 12px', color:'#EDE8DC', whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(r.rev)).toLocaleString()}</td>
                  <td style={{ padding:'9px 12px', color:'#C9A84C', fontWeight:600, whiteSpace:'nowrap' }}>{'₦'+Number(Math.round(r.profit)).toLocaleString()}</td>
                  <td style={{ padding:'9px 12px', color:'#C9A84C', fontWeight:600, whiteSpace:'nowrap' }}>{m}%</td>
                </tr>
              )
            })}
          </tbody>
