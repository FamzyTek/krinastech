...
export default function StaffDashboard({ currentUser, products, sales, page, setPage, cart, setCart, onRecordSale, onLogout }) {
  ...
  function addToCart(product) {
    ...
    if(ex){if(ex.qty>=product.qty)return;setCart(cart.map(c=>c.id===product.id?{...c,qty:c.qty+1}:c))}
      ...
      <div style={{ background:k2, border:`1px solid ${gold}`, borderRadius:14, padding:24, width:'100%', maxWidth:320 }}>
        ...
        {receipt.items.map((it,i)=>(
          <div key={i} style={{ display:'flex', justifyContent:'space-between', fontSize:13, padding:'5px 0', borderBottom:'1px solid #1A1A1A', color:w }}>
            <span>{it.name}{it.qty>1?' x'+it.qty:''}</span><span style={{ color:gold, fontWeight:600 }}>{fmt(it.price)}</span>
          </div>
        ))}
  ...
  return(
    <div style={{ minHeight:'100vh', background:'#0A0A0A', display:'flex', flexDirection:'column', overflow:'hidden' }}>

      {/* TOPBAR */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 16px', height:52, background:k2, borderBottom:'1px solid #222', flexShrink:0 }}>
      ...
      <div style={{ flex:1, overflowY:'auto', overflowX:'hidden', padding: mobile ? '12px 12px 80px' : '16px', WebkitOverflowScrolling:'touch' }}>
        ...
        {page === 'pos' && (
          <div>
            ...
            {mobile ? (
              /* MOBILE POS — stacked layout */
              <div>
                {/* Cart summary bar at top when items in cart */}
                {cart.length > 0 && (
                    ...
                    <div style={{ padding:'8px 14px' }}>
                      {cart.map(item=>(
                        <div key={item.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 0', borderBottom:'1px solid #1A1A1A' }}>
                          ...
                          <span onClick={()=>setCart(cart.filter(c=>c.id!==item.id))} style={{ color:'#E07070', cursor:'pointer', fontSize:18, marginLeft:4 }}>✕</span>
                        </div>
                      ))}
                    </div>
              ...
              /* DESKTOP POS — side by side */
              <div style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:14, minHeight:480 }}>
                <div>
                ...
                <div style={{ display:'flex', flexDirection:'column', background:k2, border:'1px solid #222', borderRadius:12, overflow:'hidden' }}>
                  <div style={{ padding:'14px', borderBottom:'1px solid #222', fontWeight:700, color:gold, fontSize:13 }}>🛒 Current Sale</div>
                  <div style={{ flex:1, padding:'10px 14px', minHeight:200 }}>
                    {cart.length===0
                      ...
                      :cart.map(item=>(
                        <div key={item.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 0', borderBottom:'1px solid #1A1A1A' }}>
                          <div style={{ flex:1, fontSize:12, color:w }}>{item.name}<br/><span style={{ fontSize:11, color:mu }}>{fmt(item.sell)} each</span></div>
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  ...
                  <div style={{ borderTop:'1px solid #222', padding:14 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12 }}>
                      <span style={{ fontSize:12, color:mu }}>Total</span>
                    ...
                    <button onClick={handleConfirmSale} disabled={cart.length===0}
                      style={{ width:'100%', padding:12, background:cart.length===0?'#333':gold, color:cart.length===0?mu:'#0A0A0A', border:'none', borderRadius:10, fontSize:13, fontWeight:700, cursor:cart.length===0?'not-allowed':'pointer' }}>
                      ✓ Confirm Sale
                    </button>
                  </div>
                </div>
        ...
        {page === 'myhistory' && (
          <div>
            <div style={{ display:'flex', overflowX:'auto', gap:10, marginBottom:16, paddingBottom:4, WebkitOverflowScrolling:'touch' }}>
              {[['My Sales',mySales.length,'Total',gold],['Revenue',fmt(mySales.reduce((a,s)=>a+s.total,0)),'All time',w],['Today',mySales.filter(s=>s.date===today).length,'Sales',w]].map(([label,val,sub,color])=>(
                <div key={label} style={{ background:k2, border:'1px solid #222', borderRadius:10, padding:14, minWidth:120, flexShrink:0 }}>
            ...
            <div style={{ background:k2, border:'1px solid #222', borderRadius:10, overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13, minWidth:380 }}>
                <thead><tr style={{ background:k3 }}>
                  {['Items','Total','Qty','Date'].map(h=><th key={h} style={{ textAlign:'left', padding:'9px 12px', fontSize:11, color:mu, borderBottom:'1px solid #222', whiteSpace:'nowrap' }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {mySales.length===0
                    ...
                    :mySales.slice().reverse().map(s=>{
                      ...
                      return(
                        <tr key={s.id} style={{ borderBottom:'1px solid #1A1A1A' }}>
                          ...
                          <td style={{ padding:'9px 12px', color:gold, fontWeight:600, whiteSpace:'nowrap' }}>{fmt(s.total)}</td>
                          <td style={{ padding:'9px 12px', color:mu, whiteSpace:'nowrap' }}>{qty}</td>
        ...
        {page === 'stockview' && (
          <div>
            <div style={{ fontSize:12, fontWeight:600, color:gold, letterSpacing:2, textTransform:'uppercase', marginBottom:12 }}>Available stock</div>
