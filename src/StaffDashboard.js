        ...
        {page === 'pos' && (
          <div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 280px', gap:14, minHeight:480 }}>
              <div>
                <input placeholder="Search product..." onChange={e => {}} style={{ width:'100%', padding:'10px 14px', background:k2, border:'1px solid #333', borderRadius:10, color:w, fontSize:14, outline:'none', marginBottom:12 }} />
        ...
        {page === 'myhistory' && (
          <div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:10, marginBottom:16 }}>
              {[['My Transactions', mySales.length, 'Total', gold], ['Revenue Generated', fmt(mySales.reduce((a,s)=>a+s.total,0)), 'All time', w], ['Today Sales', mySales.filter(s=>s.date===today).length, 'Transactions', w]].map(([label, val, sub, color]) => (
                <div key={label} style={{ background:k2, border:'1px solid #222', borderRadius:10, padding:14 }}>
            ...
            <div style={{ background:k2, border:'1px solid #222', borderRadius:10, overflow:'hidden' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                <thead><tr style={{ background:k3 }}>
                  {['Items','Total','Qty','Date'].map(h => <th key={h} style={{ textAlign:'left', padding:'9px 12px', fontSize:11, color:mu, borderBottom:'1px solid #222', letterSpacing:1, textTransform:'uppercase' }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {mySales.length === 0
                    ...
                    : mySales.map(s => {
                      ...
                      return (
                        <tr key={s.id} style={{ borderBottom:'1px solid #1A1A1A' }}>
                          <td style={{ padding:'9px 12px', color:w, maxWidth:200, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={items}>{items}</td>
                          <td style={{ padding:'9px 12px', color:w }}>{fmt(s.total)}</td>
                          <td style={{ padding:'9px 12px', color:w }}>{qty}</td>
